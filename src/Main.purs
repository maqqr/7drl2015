...
data GameState = Game { level :: Level, player :: Creature, npcs :: [Creature], items :: [Item], playerName :: String, points :: Number, inventory :: [Item] }
               | MainMenu
               | NameCreation { playerName :: String }
               | CharCreation { playerName :: String }

initialState :: String -> GameState
initialState pname = Game { level: stringToLevel testLevel, player: pl, npcs: [testGuard], items: [testItem1, testItem2], playerName: pname, points: 0, inventory: [] }
    where
        pl :: Creature
...
pickUp point (Game state) =
    case head (filter (\i -> i.pos == point) state.items) of
        Just item -> return $ Game state { items = deleteItem point state.items, inventory = addItem item state.inventory }
        Nothing   -> return (Game state)
    where
        ...
        addItem xs { itemType: Loot { value: num }, pos: poi, weight: num } = xs
        addItem xs x = x:xs
...
onKeyPress console st@(Game state) key                     | key == 80      = return 
